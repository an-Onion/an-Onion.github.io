...
这些场景的共性是：按钮触发的是**一组命令或操作**，而非从列表中**选择一个值**。这一语义差异是 Menu Button 与 Select、Combo Box 等模式的核心区别。
...
**关键区分**：Menu Button 的菜单项执行**操作**（如复制、删除），而 Select 的选项代表**值**（如选择字体大小）。这一语义差异直接影响辅助技术的朗读方式和用户的交互预期。
...
4. **库的选择**：常规场景用 `element.focus()`（Radix/shadcn），大数据用 `aria-activedescendant`（React Aria）
