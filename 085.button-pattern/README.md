...
**角色声明**是基础要求。按钮元素的 role 属性应设置为 button，向辅助技术表明这是一个按钮组件。对于使用 button 这样的原生 HTML 元素，浏览器会自动处理角色声明，无需开发者手动添加。

**可访问名称**是按钮最重要的可访问性特征之一。按钮必须有可访问的名称，这个名称可以通过多种方式提供：按钮内部的文本内容是最常见的来源；在某些情况下，可以使用 aria-labelledby 引用页面上的其他元素作为标签；或者使用 aria-label 直接提供标签文本。屏幕阅读器用户主要依赖这个名称来理解按钮的功能。

**描述信息**可以通过 aria-describedby 属性关联。如果页面上存在对按钮功能的详细描述说明，应将描述元素的 ID 赋给这个属性，辅助技术会在播报按钮名称后继续播报描述内容。

**禁用状态**需要正确使用 aria-disabled 属性。当按钮的关联操作不可用时，应设置 aria-disabled="true"。这个属性向辅助技术传达按钮当前处于禁用状态，用户无法与之交互。需要注意的是，对于原生 HTML button 元素，应使用 disabled 属性而非 aria-disabled。

**切换状态**使用 aria-pressed 属性来传达，这个属性只用于实现为切换按钮的组件。属性值应为 true（按下状态）、false（未按下状态）或 mixed（部分选中状态，用于三态树节点等场景）。
...
这是最简单也是最推荐的实现方式。原生 button 元素已经具备正确的角色、键盘支持和默认行为。
...
这个示例展示了如何实现一个切换按钮。无论状态如何变化，按钮的标签始终保持为「静音」，通过 aria-pressed 属性传达当前状态。
